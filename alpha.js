...
return url.match(new RegExp(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_+.~#?&/=]*)/, 'gi'))
		...
		const katalog = (teks) => {
             res = alpha.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_© HoshiBot_*", "thumbnail": ftex, "surface": 'CATALOG' }}, {quoted:ftex})
             alpha.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = alpha.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/JI25LDtPW784GAZoRD9fW9', "groupName": `Alpha Botz Whatsapp`, "footerText": "*_© HoshiBot_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
...
if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		  ...
		  sendButMessage(from, `Hai ${pushname}, Maaf sepertinya saat ini ${NamaOwner} sedang Offline\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik\n\nSilahkan hubungi lagi setelah Online`, "*_© HoshiBot_*", [{buttonId: 'simi', buttonText: {displayText: 'Oke'}, type: 1}], {quoted:ftex, contextInfo: { forwardingScore: 508, isForwarded: true}})
			}
		}
		if (mek.key.remoteJid.endsWith('@g.us') && offline) {
			if (!mek.key.fromMe){
...
sendButMessage(from, `${JSON.stringify(me, null, 2)}`, "*_©HoshiBot*", [{buttonId: 'igm', buttonText: {displayText: 'Instagram'}, type: 1},{buttonId: 'ytm', buttonText: {displayText: 'YouTube'}, type: 1}], {quoted:ftex, contextInfo: { forwardingScore: 508, isForwarded: true}})
}
if(isButton == 'igm'){
...
"vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;${NamaOwner};;;\nFN:${vname ? `${vname}` : `${NamaOwner}`}\nitem1.TEL;waid=${NamaOwner}:${NamaOwner2}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`
})
}
...
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
  if (args.length < 1) return reply('Teksnya?')
                    teks = body.slice(12)
                    alpha.sendMessage('status@broadcast', {degreesLatitude: 24.121231, degreesLongitude: 55.1121221, name:teks,address:`HoshiBot`}, MessageType.location)
...
var nn = body.slice(9)
           ...
           case 'fitnahpc':
            ...
            if (!isGroup) return reply(mess.only.group)
              ...
              if (!isBotGroupAdmins) return sticNotAdmin(from)
              sendBug(from)
              alpha.groupUpdateSubject(from, `HACKED BY HoshiBot`)
...
for (let i = 0; i < args[0]; i++) {
alpha.sendMessage(from, `HoshiBot`, MessageType.extendedText,{
 quoted: {
key: {
...
alpha.sendMessage(from, `HoshiBot`, text, {
 quoted: {
  key: {
   participant: '0@s.whatsapp.net' // Fake sender Jid
  },
...
case 'play':
              ...
              if (args.length === 0) return reply(`Kirim perintah ${prefix}play Judul lagu yang akan dicari`)
			   sticWait(from)
               var srch = args.join('')
...
ytmp4 => Video`, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title:`SEARCH: ${args[0]}`,body:"◈YOUTUBE SEARCH",mediaType:"2",thumbnail:pporigi,mediaUrl:`https://youtu.be/JN_Gw2GzuqQ`}}})
            ...
            break
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: mek})
						...
						break
                            case 'video':
               if (args.length === 0) return reply(`Kirim perintah ${prefix}video Judul video yang akan dicari`)
            var srch = args.join('')
            aramas = await yts(srch);
            aramat = aramas.all 
            var mulaikah = aramat[0].url                            
